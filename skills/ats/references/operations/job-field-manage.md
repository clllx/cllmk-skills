...
本路由通过自然语言在 Moka ATS 中**创建**或**查询**职位自定义字段，使用 `cllmk curl`：
...
> 本路由只覆盖查询和创建。修改、停用或删除职位字段的真实接口尚未确认，遇到这些意图时停止写入并说明不在本 skill 覆盖范围，不得猜测 endpoint 或 payload。
...
| 修改/停用/删除字段 | 「修改」/「改名」/「停用」/「删除字段」 | 不在本 skill 覆盖范围；停止写入 |
