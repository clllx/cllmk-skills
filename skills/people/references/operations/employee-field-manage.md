---
name: employee-field-manage
description: People 员工信息设置的字段管理 —— 查询档案分类/分组/字段、创建自定义字段、编辑、停用、启用
---
...
**上表之外的 type 一律不支持创建。** 用户要求建部门选择器、员工选择器、职级、银行卡等类型时，停下说明：这些是系统内置类型或未实证的形态，本 skill 不支持创建，请在页面手工操作。完整类型清单见附录 A。
...
**银行卡组合字段(40) 不支持创建**：其子属性数量很大，全量 payload 可能达到数百 KB，且创建 body 尚未验证。用户要求时停下说明，请在页面手工操作。
...
以下能力**当前不支持**，用户提出时停下说明并建议在页面手工操作：
